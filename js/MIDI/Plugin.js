...
(function()
	...
	if (window.AudioContext || window.webkitAudioContext)
		(function()
			...
			var audioBuffers =
				{};
			var audioLoader = function(instrument, urlList, index, bufferList,
					callback)
				{
			...
			root.chordOn = function(channel, chord, velocity, delay)
				...
				for ( var n = 0, length = chord.length; n < length; n++)
					{
					ret[note = chord[n]] = root.noteOn(channel, note, velocity,
							delay);
					}
				return ret;
				};
			...
			root.connect = function(conf)
				...
				for ( var instrument in MIDI.Soundfont)
					...
					for ( var i = 0; i < urlList.length; i++)
						{
						audioLoader(instrument, urlList, i, bufferList,
								oncomplete);
						}
					}
				};
	...
	if (window.Audio) (function()
		...
		var channelInstrumentNoteIds = []; // instrumentId + noteId that is
											// currently playing in each
											// 'channel', for routing
											// noteOff/chordOff calls
		var notes =
			{}; // the piano keys
	...
	(function()
		...
		root.connect = function(instruments, conf)
			...
			soundManager.url = conf.soundManagerSwfUrl
			...
			soundManager.onload = function()
				...
				var createBuffer = function(instrument, id, onload)
					...
					notes[instrumentId + "" + id] = soundManager.createSound(
						{
						id : id,
						url : MIDI.soundfontUrl + instrument + "-mp3/" + id
								+ ".mp3",
						multiShot : true,
						autoLoad : true,
						onload : onload
	...
	MIDI.GeneralMIDI = (function(arr)
		...
		var clean = function(v)
			{
			return v.replace(/[^a-z0-9 ]/gi, "").replace(/[ ]/g, "_")
					.toLowerCase();
			};
		var ret =
			{
		...
		for ( var key in arr)
			...
			for ( var n = 0, length = list.length; n < length; n++)
				...
				var num = parseInt(instrument
						.substr(0, instrument.indexOf(" ")), 10);
		...
		return ret;
			...
			(
				...
				'Piano' :
					[
					'1 Acoustic Grand Piano', '2 Bright Acoustic Piano', '3 Electric Grand Piano', '4 Honky-tonk Piano', '5 Electric Piano 1', '6 Electric Piano 2', '7 Harpsichord', '8 Clavinet'
					],
				...
				'Ensemble' :
					[
					'49 String Ensemble 1', '50 String Ensemble 2', '51 Synth Strings 1', '52 Synth Strings 2', '53 Choir Aahs', '54 Voice Oohs', '55 Synth Choir', '56 Orchestra Hit'
					],
				'Brass' :
					[
				...
				'Percussive' :
					[
					'113 Tinkle Bell', '114 Agogo', '115 Steel Drums', '116 Woodblock', '117 Taiko Drum', '118 Melodic Tom', '119 Synth Drum'
					],
				'Sound effects' :
					[
					'120 Reverse Cymbal', '121 Guitar Fret Noise', '122 Breath Noise', '123 Seashore', '124 Bird Tweet', '125 Telephone Ring', '126 Helicopter', '127 Applause', '128 Gunshot'
					]
				});
