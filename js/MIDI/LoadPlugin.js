...
(function()
	...
	MIDI.loadPlugin = function(conf)
		...
		if (typeof (instruments) !== "object") instruments =
			[
				instruments
			];
		// /
	...
	connect.flash = function(filetype, instruments, conf)
		{
		// fairly quick, but requires loading of individual MP3s (more http
		// requests).
		if (MIDI.loader) MIDI.loader.message("Flash API...");
		DOMLoader.script.add(
			{
			src : conf.soundManagerUrl
	...
	connect.audiotag = function(filetype, instruments, conf)	// Firefox
		{
			...
			getNext : function(instrumentId)
				...
				if (USE_XHR)
					...
					DOMLoader.sendRequest(
						{
						url : MIDI.soundfontUrl + instrumentId + "-" + filetype
								+ ".js",
						onprogress : getPercent,
						onload : function(response)
							{
							MIDI.Soundfont[instrumentId] = JSON
					...
					DOMLoader.script
							.add(
								{
								src : MIDI.soundfontUrl + instrumentId + "-"
										+ filetype + ".js",
								verify : "MIDI.Soundfont." + instrumentId,
								callback : function()
									{
									if (MIDI.loader)
										MIDI.loader.update(null,
												"Downloading...", 100);
									queue.getNext();
									}
								});
	...
	connect.webaudio = function(filetype, instruments, conf)	// Chrome
		{
			...
			getNext : function(instrumentId)
				...
				if (USE_XHR)
					...
					DOMLoader
							.sendRequest(
								{
								url : MIDI.soundfontUrl + instrumentId + "-"
										+ filetype + ".js",
								onprogress : getPercent,
								onload : function(response)
									{
	...
	var getPercent = function(event)
		...
		if (!this.totalSize)
			...
			if (this.getResponseHeader("Content-Length-Raw"))
				{
				this.totalSize = parseInt(this
						.getResponseHeader("Content-Length-Raw"));
				}
			else
				{
		...
		var percent = this.totalSize ? Math.round(event.loaded / this.totalSize
				* 100) : "";
