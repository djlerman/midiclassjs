...
MIDI.File = function(timebase) {
  ...
  this.exportBinary = function() {
    var bin = "MThd\0\0\0\6\0" + String.fromCharCode((data.tracks.length > 1) ?
        ...
        switch (event.type) {
          ...
          case "meta":
            ...
            switch (event.subtype) {
              ...
              case "keySignature":
                tbin += int2varBytes(2);
                tbin += int2bytes((event.key < 0) ? (event.key + 256) : event.key,
                  1);
  ...
  this.exportText = function(relativeTS) {
    var s = "MFile " + ((data.tracks.length > 1) ? "1 " : "0 ") + data.tracks.length +
        ...
        switch (event.subtype) {
          ...
          case "smpteOffset":
            s += " " + ({
              ...
              29: 0x40,
              30: 0x60
            }[event.frameRate] | event.hour) + " " + event.min + " " + event.sec +
                ...
                switch (event.subtype) {
                  ...
                  case "sequencerSpecific":
                    ...
                    for (var pi = 0; pi < line.length; pi++)
                      if (line[pi]) event.data += String.fromCharCode(parseInt(
                        line[pi], 16));
                    break;
          ...
          case "meta":
            ...
            switch (event.subtype) {
              ...
              case "smpteOffset":
                ...
                break;
              case "timeSignature":
                s += " Numerator=\"" + event.numerator + "\" LogDenominator=\"" +
                  (Math.log(event.denominator) / Math.LN2) +
              ...
              default:
                if (event.text !== undefined) {
                  s += ">" + escapeHtml(event.text) + "</" + subtypes[event.subtype]
                    .xml + ">";
                } else if (event.data !== undefined) {
                  s += ">";
  ...
  this.transposeTrack = function(trackNumber, steps) {
    ...
    for (var ei = track.length; ei--;)
      if ((track[ei].subtype == "noteOff") || (track[ei].subtype == "noteOn"))
        track[ei].noteNumber = Math.max(Math.min(track[ei].noteNumber + steps,
          127), 0);
  };
