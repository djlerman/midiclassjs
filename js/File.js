...
(function(MIDI) {
  ...
  MIDI.File = function(timebase) {
      ...
      for (var ti = data.tracks.length; ti--;)
        for (var ei = data.tracks[ti].length; ei--;)
          if (data.tracks[ti][ei].subtype == "programChange")
            MIDI.programChange(data.tracks[ti][ei].channel, data.tracks[ti][
              ei
            ].programNumber);
    ...
    this.exportBinary = function() {
      var bin = "MThd\0\0\0\6\0" + String.fromCharCode((data.tracks.length >
          1) ?
        1 : 0) + int2bytes(data.tracks.length, 2) + int2bytes(data.header.ticksPerBeat,
        2);
          ...
          switch (event.type) {
            ...
            case "meta":
              ...
              switch (event.subtype) {
                ...
                case "keySignature":
                  tbin += int2varBytes(2);
                  tbin += int2bytes((event.key < 0) ? (event.key + 256) :
                    event.key,
                    1);
     ...
     */
    this.importBinary = function(blob) {
      data = MidiFile(blob);
    };
    ...
    this.exportText = function(relativeTS) {
      var s = "MFile " + ((data.tracks.length > 1) ? "1 " : "0 ") + data.tracks
        .length +
        " " + data.header.ticksPerBeat + "\n";
      for (var ti = 0; ti < data.tracks.length; ti++)
    ...
    this.exportMsgText = function(trackNumber, messageNumber) {
      ...
      if (!event) throw "No such message: " + trackNumber + "/" +
        messageNumber;
          ...
          switch (event.subtype) {
            ...
            case "smpteOffset":
              s += " " + ({
                ...
                29: 0x40,
                30: 0x60
              }[event.frameRate] | event.hour) + " " + event.min + " " +
                event.sec +
                " " + event.frame + " " + event.subframe;
              break;
            case "timeSignature":
              s += " " + event.numerator + "/"
              //              + Math.log(event.denominator)/Math.LN2
              + event.denominator + " " + event.metronome + " " + event.thirtyseconds; /////!!!
              break;
            case "keySignature":
    ...
    function importEventText(str) {
      ...
      var event = {};
      if ((event.deltaTime = parseInt(line[0])) === NaN) throw "Bad timestamp: " +
        line[0];
          ...
          if (enode.localName == "Event") {
            ...
            for (var vnode = enode.firstChild; vnode; vnode = vnode.nextSibling)
              switch (vnode.localName) {
                case "Absolute":
                  event.deltaTime = Math.max(parseInt(vnode.textContent) -
                    lastTime, 0);
                ...
                default:
                  ...
                  function checkInt(n) {
                    ...
                    if (v === NaN) throw event.subtype + ", " + n +
                      ": number expected, found: " + v;
                    return v;
                  }

                  if (event.type == "channel") event.channel = checkInt(
                    "Channel");
                  ...
                  switch (event.subtype) {
                    ...
                    case "timeSignature":
                      ...
                      event.denominator = Math.pow(2, checkInt(
                        "LogDenominator"));
                      event.metronome = checkInt(
                        "MIDIClocksPerMetronomeClick");
                      event.thirtyseconds = checkInt(
                    ...
                    case "sequencerSpecific":
                      ...
                      for (var pi = 0; pi < line.length; pi++)
                        if (line[pi]) event.data += String.fromCharCode(
                          parseInt(
                            line[pi], 16));
                      break;
    ...
    this.exportXML = function(relativeTS) {
      var s = "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>\n" +
        ...
        "<TimestampType>" + (relativeTS ? "Delta" : "Absolute") +
        "</TimestampType>\n";
        ...
        for (var ei = 0; ei < track.length; ei++) {
          ...
          switch (event.type) {
              ...
              switch (event.subtype) {
                ...
                case "smpteOffset":
                  s += " TimeCodeType=\"1\" Hour=\"" + ({
                    ...
                    29: 0x40,
                    30: 0x60
                  }[event.frameRate] | event.hour) + "\" Minute=\"" + event.min +
                    "\" Second=\"" + event.sec + "\" Frame=\"" + event.frame +
                ...
                case "timeSignature":
                  ...
                  break;
                case "keySignature":
                  s += " Fifths=\"" + event.key + "\" Mode=\"" + event.scale +
                    "\"/>";
                  break;
                default:
                  if (event.text !== undefined) {
                    s += ">" + escapeHtml(event.text) + "</" + subtypes[event
                      .subtype]
                      .xml + ">";
                  } else if (event.data !== undefined) {
                    s += ">";
            ...
            case "channel":
              ...
              s += "\t\t<" + subtypes[event.subtype].xml + " Channel=\"" +
                event.channel +
                "\"";
              ...
              switch (event.subtype) {
                case "noteOff":
                case "noteOn":
                  s += " Note=\"" + event.noteNumber + "\" Velocity=\"" +
                    event.velocity +
                    "\"/>";
                  break;
                case "noteAftertouch":
                  s += " Note=\"" + event.noteNumber + "\" Pressure=\"" +
                    event.amount +
                    "\"/>";
                  break;
                case "controller":
                  s += " Control=\"" + event.controllerType + "\" Value=\"" +
                    event.value + "\"/>";
                  break;
                case "programChange":
    ...
    this.transposeTrack = function(trackNumber, steps) {
      ...
      for (var ei = track.length; ei--;)
        if ((track[ei].subtype == "noteOff") || (track[ei].subtype ==
          "noteOn"))
          track[ei].noteNumber = Math.max(Math.min(track[ei].noteNumber +
            steps,
            127), 0);
    };
    ...
    this.deleteTrack = function(trackNumber) {
      if (trackNumber >= data.tracks.length) throw "No such track: " +
        trackNumber;
    ...
    this.soloTrack = function(trackNumber) {
      if (trackNumber >= data.tracks.length) throw "No such track: " +
        trackNumber;
      data.tracks = trackNumber ? [data.tracks[0], data.tracks[trackNumber]] : [
        data.tracks[0]
      ];
